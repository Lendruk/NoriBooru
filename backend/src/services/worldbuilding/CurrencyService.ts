...
export class CurrencyService extends VaultService {
	...
	public async getCurrency(id: string): Promise<WorldCurrency> {
		const rawCurrency = await this.db.query.worldCurrencies.findFirst({
			where: eq(worldCurrencies.id, id)
		});
	...
	public async updateCurrency(
		...
		const updatedCurrency = await this.db
			.update(worldCurrencies)
			.set({ name, weight, description, updatedAt: Date.now() })
			.where(eq(worldCurrencies.id, id))
			.returning();
