...
const createPlaylist = async (request: Request, reply: FastifyReply) => {
	...
	const playlist = (await db.insert(playlists).values({ name: body.name, createdAt: Date.now(), randomizeOrder: body.randomizeOrder ? 1 : 0, timePerItem: body.timePerItem }).returning())[0];
