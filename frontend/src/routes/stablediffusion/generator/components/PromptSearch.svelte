...
>
	<div on:click={(e) => e.stopPropagation()} class="bg-zinc-900 min-h-[80%] min-w-[80%] p-2">
		<div class="text-4xl pb-2">Search saved prompts</div>
		<div class="flex">
			<div class="flex-[0.25]">
				{#each prompts as prompt}
					<div class="flex justify-between">
						<Button
							class="rounded-none h-[40px] flex flex-1"
							onClick={() => {
								currentOpenPrompt = prompt;
								currentOpenPromptName = prompt.name;
							}}>{prompt.name ?? 'unnamed'}</Button
						>
						<Button class="rounded-none h-[40px]" onClick={() => deleteSavedPrompt(prompt.id!)}
							><TrashIcon /></Button
						>
					</div>
				{/each}
			...
			<div class="flex flex-1 flex-col pl-2">
				{#if currentOpenPrompt}
					<div>
						<div>
							<LabeledComponent>
								<div slot="label">Prompt name</div>
								<div slot="content" class="flex">
									<input type="text" bind:value={currentOpenPromptName} />
									<Button onClick={() => updatePrompt()}>Update</Button>
								</div>
							</LabeledComponent>
							<LabeledComponent>
								<div slot="label">Positive Prompt</div>
								<div slot="content">
						...
						<Button
							class="h-[40px]"
							onClick={() => { onSelectPrompt(currentOpenPrompt!); isOpen = false; }}>Select</Button
						>
					</div>
				{:else}
					<div class="text-8xl">No Prompt selected</div>
				{/if}
			</div>
		</div>
	</div>
